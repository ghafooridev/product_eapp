...
const UploadImage: FC<{ productId: string }> = ({ productId }) => {
  ...
  const updateImageList = (imageId: string) => {
    setImages((prevImages) => prevImages?.filter(img => img.id !== imageId) || null)
  }
            ...
            {images?.map(item => {
              return (
                <div className="relative group " key={item.id}>
                  ...
                  <Image width={100} height={100} src={item.image} alt="product image" className="mt-4 mx-auto rounded-md" />
                </div>
              )
            })}
          </div>}
